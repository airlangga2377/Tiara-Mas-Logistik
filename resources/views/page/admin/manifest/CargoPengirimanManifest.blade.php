            ...
            <div class="col-xl-8 col-md-0 col-sm-0 shadow-sm p-3" id="tableIdParent"> 
                ...
                <table id="tableId" class="display">
                    ...
                    @foreach ($allCargo as $barang) 
                        @if ($barang->no_manifest && !($barang->jenis_pengiriman != $jenisUser && !Auth::user()->is_user_superadmin)) 
                            <tr>
                                <td data-toggle="tooltip" data-placement="top" title="Jenis Pengiriman {{ $barang->jenis_pengiriman }}">{{ $index }}</td> 
                                @php
                                    $index++
                                @endphp
                                ...
                                @if ($barang->jenis_pengiriman == "truk" || $name == "superadmin")
                                    <td>
                                        <div class="row justify-content-start align-items-center g-2 px-3">
                                            @if ($barang->last_id_message_tracking == 1  && ($barang->asal == $kodeKota->id_kode_kota || $name == "superadmin")) 
                                            ...
                                            @if (($barang->last_id_message_tracking == 2) && ($barang->tujuan == $kodeKota->id_kode_kota || $name == "superadmin")) 
                                                <div class="col-6 text-center">
                                                        <form action="{{ url("barang/manifest/sampai") }}" method="post">
                                                            <input type="text" name="no_manifest" value="{{ encrypt($barang->no_manifest) }}" hidden>
                                ...
                                @if ($barang->jenis_pengiriman == "bus" || $name == "superadmin")
                                    <td>
                                        <div class="row justify-content-start align-items-center g-2 px-3">
                                            @if ($barang->last_id_message_tracking == 1  && ($barang->asal == $kodeKota->id_kode_kota || $name == "superadmin")) 
                                            <div class="col-6 text-center">
